setTimeout(() => {
    class contentScript {
        ...
        idAnalisaCheck() {
            if (!this.idAnalisa) {
                const email = $('.gD:first').attr('email');
                const name = $('.gD:first').attr('name');
        ...
        injectCode(time) {
            setTimeout(() => {
                $('.AO .nH:first').before(`
                <div class="shielded-container" id="mainDiv2">
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    ...
                                    if (weightedAverageScore < 1.5) {
                                        ...
                                        $('.a3s a[data-shielded="shielded_link' + index + '"]').attr('data-lowrisk', 'true');
                                        ...
                                        $('.a3s a[data-shielded="shielded_link' + index + '"]').attr('data-mediumrisk', 'true');
                                        $('.a3s a[data-shielded="shielded_link' + index + '"]').attr('data-harmless', response.data.harmless);
                                        ...
                                        $('.a3s a[data-shielded="shielded_link' + index + '"]').attr('data-highrisk', 'true');
                                        $('.a3s a[data-shielded="shielded_link' + index + '"]').attr('data-harmless', response.data.harmless);
                                        $('.a3s a[data-shielded="shielded_link' + index + '"]').attr('data-malicious', response.data.malicious);
                                        $('.a3s a[data-shielded="shielded_link' + index + '"]').attr('data-suspicious', response.data.suspicious);
                                        $('.a3s a[data-shielded="shielded_link' + index + '"]').attr('data-undetected', response.data.undetected);
        ...
        analisaFile() {
            const thisClass = this;
            var datas = [];
            $('.aZo .e').each(function (index) {
                var url = $(this).attr('href');
                var preview = $(this).find('span:first').text();
            ...
            $('.aZo').each(function (index) {
                $(this).attr('data-shielded', 'shielded_file' + (index + 1));
            });
            console.log(datas);
            if (datas.length == 0) {
                console.log('kaga ada file attachment');
                $('#sectionAnalisaFile').html('<ul style="margin-top: 10px;"> Tidak ada file attachment. </ul>')
            } else {
                thisClass.idAnalisaCheck();
                ...
                chrome.storage.local.get(['useExternal']).then(function (result) {
                    ...
                    datas.forEach(function (data, index) {
                        chrome.runtime.sendMessage({ action: 'fetchFinalUrl', url: data['url'], index: (index + 1) }, response => {
                            if (response.error) {
                                console.error('Error:', response.error);
                            } else {
                                fetchFileAndConvertToBase64(response.finalUrl, response.index).then(result => {
                                    if (result.error) {
                                        ...
                                        $.ajax({
                                                        ...
                                                        success: function (response) {
                                                            ...
                                                            if (status == 'completed') {
                                                                ...
                                                                if (total == 0) {
                                                                    ...
                                                                    $('.aZo[data-shielded="shielded_file' + index + '"]').attr('data-mediumrisk-analyze', 'true');
                                                                    $('.aZo[data-shielded="shielded_file' + index + '"]').attr('data-suspicious', suspicious);
                                                                    $('.aZo[data-shielded="shielded_file' + index + '"]').attr('data-malicious', malicious);
                                                                } else {
                                                                    ...
                                                                    $('.aZo[data-shielded="shielded_file' + index + '"]').attr('data-highrisk-analyze', 'true');
                                                                    $('.aZo[data-shielded="shielded_file' + index + '"]').attr('data-suspicious', suspicious);
                                                                    $('.aZo[data-shielded="shielded_file' + index + '"]').attr('data-malicious', malicious);
                                                                }
                                            ...
                                            error: function (jqXHR, textStatus, errorThrown) {
                                                $('#statusFile' + response.index).html(' || ' + errorThrown);
                                            },
                                            complete: function () {
                                                console.log('Done');
                                            }
                                        ...
                                        $(document).on('click', '#clickableFile', function () {
                                            ...
                                            const target = $('.aZo[data-shielded="shielded_file' + index + '"]');
                                            $('.shielded-highlight').removeClass('shielded-highlight');
                                            target.addClass('shielded-highlight');
                                            target[0].scrollIntoView({ behavior: 'smooth', block: 'center' });
                                        });
        ...
        fileClickPrevention() {
            ...
            $('.aZo button div').on('click', function (e) {
    ...
    chrome.storage.local.get(['ExtStat']).then((result) => {
        ...
        if (ExtStat == 1) {
            ...
            $('.gb_re').after('<img style="height: 100%; width: 10%;" src="https://drive.google.com/uc?id=17h2JU2sMWc3WS0yiMp96gPaxUvP55XnF">')
        }
    });
